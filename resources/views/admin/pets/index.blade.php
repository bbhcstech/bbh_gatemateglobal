...
<style>
    ...
    * {
    ...
    .stat-icon.inactive { background: #fee2e2; color: var(--danger); }
    .stat-icon.vaccinated { background: #dbeafe; color: var(--primary); }
...
function getPetImageUrl($image) {
    ...
    $possiblePaths = [
        $image, // Original path
        'uploads/pets/' . basename($image), // With uploads/pets prefix
        'pets/' . basename($image), // With pets prefix
        'storage/pets/' . basename($image), // With storage prefix
        'images/pets/' . basename($image), // With images/pets prefix
        'uploads/' . basename($image), // With uploads prefix
    ];
...
<div class="container-fluid">
    ...
    <div class="page-header">
        <div class="header-title">
            <h2>
                <i class="fas fa-paw me-2" style="color: var(--primary);"></i>
                Pet Management
            </h2>
        ...
        <div class="toolbar">
            ...
            @if(auth()->check() && strtolower(optional(auth()->user()->roleMaster)->role_name) === 'resident')
                <a href="{{ route('pets.all-residents') }}" class="btn btn-info">
                    ...
                    View All Residents' Pets
                </a>
            @endif

            <!-- Archive Button -->
            <a href="{{ route('pets.archived') }}" class="btn btn-outline">
    ...
    <div class="stats-grid">
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h6>Total Pets</h6>
                <h3 id="totalPets">{{ $totalPets ?? $pets->count() }}</h3>
            </div>
        </div>
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h6>Active</h6>
                <h3 id="activeCount">{{ $activeCount ?? $pets->where('activity_status', 1)->count() }}</h3>
            </div>
        </div>
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h6>Inactive</h6>
                <h3 id="inactiveCount">{{ $inactiveCount ?? $pets->where('activity_status', 0)->count() }}</h3>
            </div>
        </div>
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h6>Vaccinated</h6>
                <h3 id="vaccinatedCount">{{ $vaccinatedCount ?? $pets->where('vaccination_status', 'yes')->count() }}</h3>
            </div>
        </div>
    </div>
    ...
    <div class="main-card">
        ...
        <div class="table-responsive">
            ...
            @else
                <table id="petsTable" class="pet-table">
                    <thead>
                        <tr>
                            <th width="40">
                    ...
                    <tbody>
                        @foreach($pets as $key => $pet)
                            <tr id="row-{{ $pet->id }}">
                                ...
                                <td>
                                    @php
                                        $imageUrl = getPetImageUrl($pet->image);
                                    @endphp
                                    ...
                                    @if($imageUrl)
                                        <img src="{{ $imageUrl }}"
                                             ...
                                             alt="{{ $pet->pet_name }}"
                                             onerror="this.onerror=null; this.style.display='none'; this.parentElement.innerHTML='<div class=\'pet-thumb\' style=\'background: var(--light); display: flex; align-items: center; justify-content: center;\'><i class=\'fas fa-paw\' style=\'color: var(--secondary);\'></i></div>';">
                                    @else
                                        <div class="pet-thumb" style="background: var(--light); display: flex; align-items: center; justify-content: center;">
                                            <i class="fas fa-paw" style="color: var(--secondary);"></i>
                                        </div>
                                    @endif
                                </td>
                                <td>
                                    <div style="font-weight: 600;">{{ $pet->pet_name ?? '-' }}</div>
                                    ...
                                    @if($pet->is_dangerous)
                                        <span class="status-badge" style="background: #fee2e2; color: #dc2626; font-size: 0.7rem; margin-top: 0.25rem;">
                                ...
                                <td>
                                    @if(!$pet->trashed())
                                        ...
                                        @if($pet->vaccination_expiry_date && $pet->vaccination_status == 'yes')
                                            <div style="font-size: 0.7rem; color: var(--secondary); margin-top: 0.25rem;">
                                ...
                                <td>
                                    <div class="status-dropdown">
                                        <select class="status-select {{ $pet->activity_status ? 'active' : 'inactive' }}"
                                                data-pet-id="{{ $pet->id }}"
                                                onchange="updateStatus(this, {{ $pet->id }})"
                                                {{ auth()->user()->role !== 'admin' && $pet->resident_id != auth()->id() ? 'disabled' : '' }}>
                                            <option value="1" {{ $pet->activity_status == 1 ? 'selected' : '' }}>Active</option>
                                            <option value="0" {{ $pet->activity_status == 0 ? 'selected' : '' }}>Inactive</option>
                                        </select>
                                    </div>
                                </td>
                                <td>
                                    <div class="action-group">
                                        <!-- View -->
                                        <a href="{{ route('pets.show', $pet->id) }}" class="action-btn view" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>

                                        @php
                                            $canEdit = auth()->user()->role === 'admin' || $pet->resident_id == auth()->id();
                                            $canDelete = auth()->user()->role === 'admin' || ($pet->resident_id == auth()->id() && $pet->activity_status == 0);
                                        @endphp

                                        @if($canEdit)
                                            <!-- Edit -->
                                            <a href="{{ route('pets.edit', $pet->id) }}" class="action-btn edit" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                        @endif

                                        @if($canDelete)
                                            <!-- Delete (Archive) - Only allowed if pet is inactive -->
                                            <form action="{{ route('pets.destroy', $pet->id) }}"
                                                  method="POST"
                                                  class="d-inline delete-form"
                                                  onsubmit="return confirmDelete('{{ $pet->activity_status }}')">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @elseif(auth()->user()->role !== 'admin' && $pet->resident_id == auth()->id() && $pet->activity_status == 1)
                                            <span class="action-btn delete disabled" title="Cannot delete active pet. Please make it inactive first.">
                                                <i class="fas fa-archive"></i>
                                            </span>
                                            <div class="warning-text">
                                                <i class="fas fa-info-circle"></i> Deactivate first
                                            </div>
                                        @endif
                                    </div>
                                </td>
...
<script>
    ...
    // Confirm delete based on pet status
    function confirmDelete(status) {
        if (status == 1) {
            alert('Cannot delete active pet. Please make it inactive first.');
            return false;
        }
        return confirm('Archive this pet? It can be restored later.');
    }
    ...
    function bulkAction(action) {
        ...
        const message = `Archive ${selectedRows.size} selected pet(s)? They can be restored later.`;
        ...
        form.action = '{{ route("pets.bulk-delete") }}';
        document.getElementById('selectedIds').value = ids;
        form.submit();
    }

    // Update Status
    function updateStatus(selectElement, petId) {
        const status = selectElement.value;
        const originalValue = selectElement.classList.contains('active') ? '1' : '0';
        ...
        .then(data => {
            if (data.success) {
                ...
                selectElement.className = `status-select ${status == 1 ? 'active' : 'inactive'}`;

                // Update the stats counters with the new counts from server
                if (data.counts) {
                    document.getElementById('totalPets').textContent = data.counts.total;
                ...
                // Reload page after 1 second to update delete button states
                setTimeout(() => {
                    location.reload();
                }, 1000);
            } else {
        ...
        .catch(error => {
            ...
            selectElement.className = `status-select ${originalValue == 1 ? 'active' : 'inactive'}`;
    ...
    function exportTable(format) {
        ...
        table.querySelectorAll('thead th').forEach((th, index) => {
            if (index > 0 && index < 7) { // Skip checkbox and actions columns
        ...
        table.querySelectorAll('tbody tr').forEach(row => {
            ...
            // Pet Details (index 3)
            const petDetails = cells[3].textContent.trim().replace(/\s+/g, ' ');

            rowData.push(cells[1].textContent.trim()); // #
            rowData.push(petDetails); // Pet Details
            rowData.push(cells[4].textContent.trim()); // Owner & Flat
            rowData.push(cells[5].textContent.trim()); // Vaccination
            rowData.push(cells[6].textContent.trim()); // Status
    ...
    function exportToExcel(data) {
        ...
        XLSX.writeFile(wb, `pets_${new Date().toISOString().split('T')[0]}.xlsx`);
    }
